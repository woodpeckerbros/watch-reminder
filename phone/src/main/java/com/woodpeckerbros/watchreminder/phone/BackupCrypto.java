...
/** Must stay format-compatible with the Wear BackupCrypto implementation. */
final class BackupCrypto {
    ...
    static byte[] ensureEncrypted(byte[] data) throws Exception {
        return isEncrypted(data) ? data : encrypt(new String(data, StandardCharsets.UTF_8));
    }
    ...
    static String decryptToText(byte[] data) throws Exception {
        if (!isEncrypted(data)) return new String(data, StandardCharsets.UTF_8); // legacy read only
        ...
        cipher.updateAAD(header);
        return new String(cipher.doFinal(Arrays.copyOfRange(data, header.length, data.length)), StandardCharsets.UTF_8);
    }

    static boolean isEncrypted(byte[] data) {
        return data != null && data.length > MAGIC.length + 2 + IV_BYTES + 16
                && data[0] == MAGIC[0] && data[1] == MAGIC[1] && data[2] == MAGIC[2]
                && data[3] == MAGIC[3] && data[4] == VERSION && data[5] == IV_BYTES;
    }
