...
- תיקון תחנון ‏22/09: במסך זמני היום מוצג כעת רק הכיתוב המרכזי האדום „לא אומרים תחנון” ללא שורת „תחנון”. חישוב התחנון משתמש ב־`kosherjava` `TefilaRules` ובבדיקת `JewishCalendar.isAseresYemeiTeshuva()`, כך שעשרת ימי תשובה, כולל השבוע שלפני יום כיפור והימים שאחריו לפי כללי הספרייה, אינם מסומנים בטעות כאמירת תחנון. נוספו בדיקות. `:app:testDebugUnitTest :app:assembleDebug` עברו.
