...
- משימה מיידית: ליצור Smart Alarm קצר, להכניס את OnePlus Watch 3 למצב שינה ולבדוק אם אזעקת המערכת השקטה מפעילה רטט בשעת היעד.
...
- ניסוי 02/09: הגדרת opt-in "גיבוי רטט של השעון" מנסה ליצור מראש דרך `ACTION_SET_ALARM` אזעקת מערכת שקטה עם רטט לשעת היעד. הגשר מופעל רק אם `resolveActivity` מחזיר Clock כברירת־מחדל ל־API הציבורי; אם הוא מחזיר Resolver, הוא מדלג כדי שלא ייפתח חלון בחירה או מסך הוספת אזעקה באמצע התראה/נודניק. ב־OnePlus Watch 3 הנוכחי ברירת־המחדל היא Resolver, ולכן אין עדיין גיבוי רטט זה.
